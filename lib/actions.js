...
export async function bookmark(tweetId) {
  ...
  const data = await fetch(`${backendURL}/sapi/tweet/${tweetId}/bookmark`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${token}`,
    },
...
export async function postTweet(content, gif = undefined) {
  const token = localStorage.getItem("token");
  ...
  const data = await fetch(`${backendURL}/api/tweet`, {
    ...
    body: JSON.stringify({
      content: content,
      giphyUrl: gif ? gif.images.original.url : undefined,
    }),
  }).then((res) => res.json());
